	...
	public Login() {
		...
		btnJoinUs.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				DBConnection controlUsername = new DBConnection("SELECT * FROM `Users` WHERE USERNAME = '" + 
						textField_UserName.getText() + "';", "User");
				DBConnection controlEmail = new DBConnection("SELECT * FROM `Users` WHERE E-MAIL ='" + 
						textField_Email.getText() + "';", "User");
				if(controlUsername.getUser() != null){ //Makes sure the username doesn't already exist in the DB
				...
				else if(controlEmail.getUser() != null){ //Makes sure that the email doesn't already exist in the DB
					Component frame = null;
					JOptionPane.showMessageDialog(frame,
						    "Your e-mail is already linked to an account",
						    "Error",
						    JOptionPane.PLAIN_MESSAGE);
				}
				else{ //Insearts a new user
				
				 DBConnection NewUser = new DBConnection("INSERT INTO `Users` (USERNAME, PASSWORD, E-MAIL, ADMINISTRATOR) " 
							+ "VALUES('" + textField_UserName.getText() + "', '" + textField_Password.getText() + "', '" + textField_Email.getText() +
							"', 0);", "Update");
				
				FirstPage.workPage = new WorkPage(new InternalWorkPage(new MenuUserLogged(), new LatestMovies()));
		...
		lblForgotPassword.setForeground(Color.BLUE);
			...
			btnSignIn.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (login.getUser() == null){ //If username or password is invalid
						Component frame = null;
						JOptionPane.showMessageDialog(frame,
					...
					else if(login.getUser().isAdministrator()){ //If the user is an administator
		
									FirstPage.workPage = new WorkPage(new InternalWorkPage(new MenuAdm(), new AddingMovies())); // Initilize the WorkPage for the admin (Not entirely true since I haven't looked at rema's code yet. But it's the correct menu)
									FirstPage.workPage.setVisible(true);
									dispose();
						
					}
					else{ //If the user exist and is not an administrator
						FirstPage.workPage = new WorkPage(new InternalWorkPage(new MenuUserLogged(), new LatestMovies())); //Initilize the WorkPage for the user with an account
						FirstPage.workPage.setVisible(true);
						dispose();
					}
